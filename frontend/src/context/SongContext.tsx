...
const server = "https://my-song-service.onrender.com";
...
export const SongProvider: React.FC<SongProviderProps> = ({ children }) => {   
  ...
  const fetchSongs = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get<Song[]>(`${server}/api/v1/song/all`,{withCredentials:true});
      setSongs(data);
  ...
  const fetchAlbums = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get<Album[]>(`${server}/api/v1/album/all`,{withCredentials:true});
      setAlbums(data);
    } catch (error) {
      console.log(error);
    } finally {
